...
// CmdContextWithLogger creates a context with a logger with the given log level and log type.
// If this returns an error consider it fat
func CmdContextWithLogger(cmd *cobra.Command, logLevel string, logType string) (context.Context, logging.Logger, error) {
	ctx := cmd.Context()
	if ctx == nil {
		ctx = context.Background()
	}
	logger := logging.GetLoggerFromContextOrNil(ctx)
	if logger == nil {
