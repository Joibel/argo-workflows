...
- Go 1.21+
- Argo Server running and accessible
- Bearer token for authentication (if auth is enabled)
...
- See `basic-workflow` for Kubernetes client approach
- See `watch-workflow` for real-time progress tracking
- See `workflow-template` for reusable templates
- See `authentication` for different auth methods
